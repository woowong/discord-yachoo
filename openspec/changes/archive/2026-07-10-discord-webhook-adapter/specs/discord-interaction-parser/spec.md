...
The system SHALL parse incoming JSON bodies into type-safe structures mapping to Discord Interaction Types (PING, ApplicationCommand, MessageComponent).
...
- **THEN** the parser SHALL extract the command name and arguments into a domain action.

#### Scenario: Parse Message Component (Type 3)
- **WHEN** the raw JSON contains `type: 3` and component custom_id `roll_dice`
- **THEN** the parser SHALL parse it as a roll request domain action.
