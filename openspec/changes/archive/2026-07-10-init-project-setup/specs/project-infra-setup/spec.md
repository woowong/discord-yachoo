...
프로젝트는 `package.json` 파일에 Effect.ts 에코시스템(`effect`, `@effect/platform`, `@effect/schema`), Cloudflare Workers 환경(`wrangler`), 개발 및 테스트 환경(`typescript`, `vitest`)을 의존성으로 정의하고 성공적으로 로드해야 한다. (SHALL)
...
프로젝트는 Effect.ts 제너레이터 및 안전한 타입 분석을 지원할 수 있도록 `tsconfig.json` 설정을 완비해야 한다. (SHALL)
