...
- **1인 / 2인 및 매치메이킹 지원:** 솔로 최고 점수에 도전하는 1인 싱글 모드, 특정 상대를 지목하는 **5분 제한 초대전 (`/yachoo challenge @유저`)**, 그리고 빠르게 대전 상대를 만나는 **오픈 매치메이킹 큐 (`/yachoo match`)**를 모두 지원합니다.
- **ELO 레이팅 및 리더보드:** 1v1 멀티플레이어 결과에 따라 변동되는 ELO 레이팅 시스템을 제공하며, 서버 리더보드가 단순 승수가 아닌 ELO 기준으로 정렬됩니다.
- **2단계 항복 제안 & 안전장치:** 대전 중 기권 버튼 클릭 시 상대방에게 항복 제안(Accept/Decline)이 전달되며, 상대가 수락해야 항복 K.O 승패가 결정되는 **2단계 항복 제안 시스템**을 탑재했습니다. 자신에 대한 결투 신청 방지 및 5개 주사위 모두 홀드 시 무의미한 롤링 차단 UI를 제공합니다.
...
- **웹 대시보드 & 분석:** Worker 루트 URL에서 접근 가능한 브라우저 기반 분석 대시보드로, ELO 레이팅 히스토리 차트(Chart.js)가 포함된 플레이어 프로필, 색상 코딩된 턴별 점수 차이(Score Diff) 열이 포함된 매치 리플레이, 전설의 경기(Legend Matches) 카탈로그, 그리고 검색 가능한 플레이어 디렉토리를 제공합니다.
- **전설의 경기 시스템:** 주목할 만한 게임 순간들을 자동으로 식별하고 태그합니다 — 극적인 역전승(25점 이상 격차 역전), 연속 고득점(5턴 연속 15점 이상), 야추 달성, 연속 뇌절(3턴 연속 0점).
- **프로필 분석:** 플레이어별 솔로/멀티 평균 점수, 최근 10경기 승/패/무 전적, ELO 레이팅 추이 차트를 추적합니다.
- **중복 게임 방지:** 진행 중인 활성 게임이 있는 상태에서 새 게임을 시작하려는 시도를 감지 및 차단하며, 기존 진행 중인 매치로의 직접 링크를 제공합니다.
...
┌─────────────────────────────────────────────────────────┐
│                 Presentation Layer                      │
│ (디스코드 웹훅 어댑터 / 콘솔 시뮬레이터 / 웹 대시보드)  │
└────────────────────────────┬────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────┐
│                 Application Layer                       │
│  (게임 워크플로우 오케스트레이션 & 전설 경기 분석)       │
└────────────────────────────┬────────────────────────────┘
                             │
┌────────────────────────────▼────────────────────────────┐
│                  Core Game Engine                       │
...
- **Application Layer (`src/application/`)**: 복잡한 게임 워크플로우(챌린지, 롤, 스코어, 항복)를 오케스트레이션하고, 매치 히스토리를 분석하여 전설의 경기를 태그합니다. 도메인 로직과 영속성·프레젠테이션 계층을 연결합니다.
- **Presentation Layer (`src/presentation/`)**:
  ...
  - `web/`: 플레이어 프로필, ELO 차트, 매치 리플레이, 전설의 경기 카탈로그를 제공하는 브라우저 기반 분석 대시보드입니다.
  - `messages/`: 디스코드 봇 응답용 한국어 메시지 템플릿 모듈입니다.
